...
Rm=mean_All(4);
RFP_derv1=diff(Rm,cp1);
RFP_derv2=diff(RFP_derv1,cp1);
 
cp1_ALL=10.^(-3:.01:.3);

RFP_Mean_num=double(subs(mean_All(4), cp1, cp1_ALL)); 
dRFP_num=double(subs(RFP_derv1,cp1,cp1_ALL));
d2RFP_num=double(subs(RFP_derv2,cp1,cp1_ALL));
[v1,i1]=min(abs(d2RFP_num(1:200)));
[v2,i2]=min(abs(d2RFP_num));
...
hold on
yyaxis  left 
plot(cp1_ALL,GFP_Mean_num,'g','linewidth',2)
ylabel('GFP Mean') 
yyaxis  right
plot(cp1_ALL,RFP_Mean_num,'r','linewidth',2)
...
subplot(2,2,3)
hold on
plot(cp1_ALL,GFP_noise_ALL_num,'color','g','linewidth',2)
plot(cp1_ALL,GFP_noise12_num,'color',[0.93,0.69,0.13],'linewidth',2)
plot(cp1_ALL,GFP_noise2_num,'color',[0.07,0.62,1.00],'linewidth',2)
xlabel('GFP Copy Number (c_{p_1})')
ylabel('GFP Noise Level')
legend('\eta_{p_1 total}','\eta_{{p_1}<-{m_1}}','\eta_{{p_1}<-{p_1}}','location','northeast')
box on
axis square
set(gca,'xscale','log')
xlim([0.01 2]) 
...
yyaxis  left 
plot(cp1_ALL,gfp_Mean_num,'g','linewidth',2)
ylabel('gfp mRNA Mean') 
yyaxis  right
plot(cp1_ALL,rfp_Mean_num,'r','linewidth',2)
xlabel('GFP Copy Number (c_{p_1})')
ylabel('rfp mRNA Mean')
box on
axis square
set(gca,'xscale','log')
xlim([0.01 2]) 